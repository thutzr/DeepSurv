...
class DeepSurv:
    ...
    **kwargs):
        ...
        """
        def saveCI(train_CI,filename):
            with open(filename,'w') as f:
                for i in range(len(train_CI)):
                    s = str(train_CI[i]).replace('[',' ').replace(']',' ')
                    s = s.replace("'",' ').replace(',',' ')+'\n'
                    f.write(s)
        ...
        if plot_loss:
            ...
            plt.show()
        if plot_train_ci:
            plt.plot(range(1,n_epochs+1),train_CI)
            plt.xlabel('Epochs')
            plt.ylabel('Training Concordance Index')
            plt.savefig('/home/kyle/Research/Traffic-Incident/train_ci.pdf')
            plt.show()
            saveCI(train_CI,'/home/kyle/Research/Traffic-Incident/train_ci.txt')
        if verbose:
